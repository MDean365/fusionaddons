...
class PokemonEvolutionScene
  ...
  def pbEvolutionSuccess(reversing=false)
    ...
    oldSpecies = GameData::Species.get(@pokemon.species)

    allOldPossibleAbilities = oldSpecies.abilities + oldSpecies.hidden_abilities
    ...
    # Check ability compatibility: if new species can have old ability, retain ability
    # If unevolved pokemon can't have its own ability, ability is randomized - retain ability
    if allNewPossibleAbilities.include?(oldAbility) || !allOldPossibleAbilities.include?(oldAbility)
