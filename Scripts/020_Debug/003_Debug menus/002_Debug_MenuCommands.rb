...
DebugMenuCommands.register("addpokemon", {
  ...
  "description" => _INTL("Give yourself an UNFUSED Pokémon of a chosen species/level. Goes to PC if party is full."),
  "effect"      => proc {
    species = pbChooseSpeciesList
    if species
      params = ChooseNumberParams.new
...
DebugMenuCommands.register("addrandompokemon", {
  "parent"      => "pokemonmenu",
  "name"        => _INTL("Add Random Pokémon"),
  "description" => _INTL("Give yourself a Random Pokémon of a chosen level. Goes to PC if party is full."),
  "effect"      => proc {
    species = rand(175560) + 421 
    if rand(100) > 70 
      species = rand(420) + 1
    end
    if species
...
DebugMenuCommands.register("randomparty", {
  "parent"      => "pokemonmenu",
  "name"        => _INTL("Give Random Party"),
  "description" => _INTL("Give 6 random pokemon, overwrite current party."),
  "effect"      => proc {
      new_party = []
      for i in 0..($Trainer.party.length() - 1) do
        if rand(100) > 30
          pkmn = Pokemon.new(rand(175560) + 421, $Trainer.party[i].level)
        else
          pkmn = Pokemon.new(rand(420) + 1, $Trainer.party[i].level)
        end
        if $Trainer.party[i].nicknamed?
          pkmn.name = $Trainer.party[i].name
        end
        new_party.push(pkmn)
      end
      $Trainer.party.clear
      new_party.each {|pkmn| $Trainer.party.push(pkmn)}
      pbMessage(_INTL("Randomized party!"))
  }
})

DebugMenuCommands.register("randomabilities", {
  "parent"      => "pokemonmenu",
  "name"        => _INTL("Randomize Party Abilities"),
  "description" => _INTL("Overwrite current party abilities with random new ones"),
  "effect"      => proc {
        $Trainer.party.each {|pkmn| pkmn.ability = rand(233) + 1}
        pbMessage(_INTL("Randomized Party Abilities!"))
  }
})
